...
extension HomeViewController {
    func createWallet() {
        ...
        AgentWrapper.shared.createWallet(withConfig: config, credentials: credentials, completion: { (error) in
            if (error != nil) {
    ...
    func openWallet() {
        let config = "{\"id\": \"\(walletName.text ?? "")\"}"
        let credentials = "{\"key\": \"\(walletName.text ?? "")\"}"
        AgentWrapper.shared.openWallet(withConfig: config, credentials: credentials) { (error, indyHandle) in
            if (indyHandle == 0){
                self.createWallet()
            }else {
    ...
    func createAndStoreDID() {
        ...
        AgentWrapper.shared.createAndStoreDid(did: did, walletHandle: handle) { (error, DID, verKey) in
            self.myDID = DID
    ...
    func shareDid() {
        ...
        AgentWrapper.shared.transport?.sendData(msg: msg, completion: { (obj) in
...
//                self.navigationController?.pushViewController(VC, animated: true)
    ...
    func getDid(){
        AgentWrapper.shared.transport?.getData(completion: { (recievedData) in
    ...
    func sendMessage() {
        ...
        AgentWrapper.shared.packMessage(message: msgData, myKey: self.myVerKey, theirKey: recieversArray, walletHandle: walletHandle) { (error, data) in
...
//                    self.navigationController?.pushViewController(VC, animated: true)
    ...
    func recieveMessage(){
        AgentWrapper.shared.transport?.getData(completion: { (recievedData) in
            ...
            AgentWrapper.shared.unpackMessage(message: msgData, myKey: self.myVerKey, theirKey: self.recievedVerKey, walletHandle: walletHandle) { (error, data) in
